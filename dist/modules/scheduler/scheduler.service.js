...
export class SchedulerService {
    ...
    scanTask = null;
    evaluationTask = null;
    constructor(config, credentials, networkMonitor) {
        this.config = config;
    ...
    async runScan() {
        try {
            ...
            this.logger.error('SchedulerService', `Scan failed: ${err.message}`);
        }
    }
    async runEvaluation() {
        try {
            ...
            const request = result.passed
                ? { type: 'success', context: { linesWritten: result.lineCount, streak: result.streak } }
                : { type: 'penalty' };
            ...
            this.logger.error('SchedulerService', `Evaluation pipeline failed: ${err.message}`);
        }
    }
}
