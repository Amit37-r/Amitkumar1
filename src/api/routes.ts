...
import { IAuthenticatedRequest } from '../modules/auth/auth.types';
...
export function registerRoutes(
  ...
  app.put('/api/config', protect, (req, res) => {
    const updates = req.body as Partial<IAppConfig>;
  ...
  // Debug: Test GitHub redirect without auth (REMOVE IN PRODUCTION)
  app.get('/api/auth/github/test', (_req, res) => {
    const url = githubOAuth.getAuthUrl('test-user-id');
    console.log('DEBUG GitHub redirect URL:', url);
    res.redirect(url);
  });
  ...
  app.get('/api/auth/github/callback', async (req, res) => {
    ...
    const result = await githubOAuth.handleCallback(code as string, userId as string);
