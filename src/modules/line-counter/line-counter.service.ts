...
import { IScanResult, IScanError, IGitNumstatEntry, IGithubCommit } from './line-counter.types';

export class LineCounterService {
  private accessor: LineCounterAccessor;
   ...
   */
  async scanAll(codeFolders: string[], githubRepos: IGithubRepo[], githubToken?: string): Promise<IScanResult> {
    const localResult = this.scanLocalFolders(codeFolders);
    const githubResult = await this.scanGithubRepos(githubRepos, githubToken);

    const combined: IScanResult = {
    ...
    try {
      ...
      this.logger.error('LineCounterService', `Failed to persist scan: ${(err as Error).message}`);
    }
  ...
  private async scanGithubRepos(repos: IGithubRepo[], token?: string): Promise<IScanResult> {
    let totalLinesAdded = 0;
    let totalLinesDeleted = 0;
    const filesChanged: string[] = [];
    const errors: IScanError[] = [];
    ...
    for (const repo of repos) {
      try {
        ...
        errors.push({ folder: `github:${repo.owner}/${repo.repo}`, message: (err as Error).message });
      }
    }
  ...
  private async fetchTodayCommits(repo: IGithubRepo, token?: string): Promise<IGithubCommit[]> {
    ...
    return (await response.json()) as IGithubCommit[];
  }

  private async fetchCommitDetails(repo: IGithubRepo, sha: string, token?: string): Promise<IGithubCommit> {
  ...
  private scanLocalFolders(codeFolders: string[]): IScanResult {
    let totalLinesAdded = 0;
    let totalLinesDeleted = 0;
    const filesChanged: string[] = [];
    const errors: IScanError[] = [];
  ...
  private scanGitFolder(folder: string): IGitNumstatEntry[] {
    const today = this.getTodayDate();
    let output = '';
  ...
  private parseNumstat(output: string): IGitNumstatEntry[] {
    const entries: IGitNumstatEntry[] = [];
  ...
  private scanNonGitFolder(folder: string): string[] {
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    return this.findRecentFiles(folder, todayStart.getTime());
  }

  private findRecentFiles(dir: string, since: number, depth = 0): string[] {
    if (depth > 5) return [];
    const files: string[] = [];
    ...
    try {
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) {
          ...
          if (!LineCounterService.CODE_EXTENSIONS.has(ext)) continue;
          try {
            if (statSync(fullPath).mtimeMs >= since) files.push(fullPath);
          } catch { /* skip */ }
        }
      }
