...
export class EmailService {
  ...
  public async sendEmail(params: ISendEmailRequest): Promise<ISendEmailResult> {
    try {
      let html: string;
      let subject: string;
  ...
  private buildPenaltyHtml(text: string, lineCount: number, target: number): string {
    return `
  ...
  private buildSuccessHtml(text: string, lineCount: number, target: number, streak: number): string {
    return `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
